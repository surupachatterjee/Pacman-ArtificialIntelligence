    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            dist_of_pos = self.getPositionDistribution(self.setGhostPosition(gameState,pos))
            for newpos,prob in dist_of_pos.items():
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        legalBoardPos = self.legalPositions
        numberOfParticles = self.numParticles
        for i in range(numberOfParticles):
            legalPos = legalBoardPos[i % len(legalBoardPos)]
    ...
    def observe(self, observation, gameState):
        ...
        # Handling ghost captured by Pacman condition
        if noisyDistance == None:
            ...
            for pos in self.legalPositions:
                dist_pac_particle = util.manhattanDistance(pacmanPosition, pos)
                if emissionModel[dist_pac_particle] > 0:
                    newBeliefDist[pos] = emissionModel[dist_pac_particle] * beliefDist[pos]
    
            # Handling condition where Total weight of new Distribution is 0
            if newBeliefDist.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                for i in range(self.numParticles):
                    self.particles[i] = util.sample(newBeliefDist)
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            updateParticles.append(util.sample(newPosDist))
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            beliefDist[particle] += 1.0/self.numParticles
        return beliefDist
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        for i in range(self.numParticles):
            updatePossPos = possiblePos[i % len(possiblePos)]
            self.particles.append(updatePossPos)
    ...
    def observeState(self, gameState):
        ...
        beliefDist =  self.getBeliefDistribution()
        newBeliefDist = util.Counter()
        ghostEaten =[] # stores index of the ghost eaten by pacman
        ...
        #ghost Probability
            ghostProb = 1
        # Ghosts that are not yet eaten by pacman
            for i in range(self.numGhosts):
                if i not in ghostEaten:
                    ghostDist = util.manhattanDistance(p[i],pacmanPosition)
                    ghostProb *= (emissionModels[i])[ghostDist]
            newBeliefDist[p] = newBeliefDist[p] + ghostProb
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                updatedGhostPos = setGhostPositions(gameState, newParticle)
                updatedGhostDistri = getPositionDistributionForGhost(updatedGhostPos, i ,self.ghostAgents[i])
                newParticle[i] = util.sample(updatedGhostDistri)
